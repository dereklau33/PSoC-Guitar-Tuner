/*******************************************************************************
* File Name: WaveGenerator_DacClk.c
* Version 2.10
*
*  Description:
...
/*******************************************************************************
* Function Name: WaveGenerator_DacClk_Start
********************************************************************************
*
* Summary:
...
void WaveGenerator_DacClk_Start(void) 
{
    ...
    WaveGenerator_DacClk_CLKEN |= WaveGenerator_DacClk_CLKEN_MASK;
	WaveGenerator_DacClk_CLKSTBY |= WaveGenerator_DacClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: WaveGenerator_DacClk_Stop
********************************************************************************
*
* Summary:
...
void WaveGenerator_DacClk_Stop(void) 
{
    ...
    WaveGenerator_DacClk_CLKEN &= (uint8)(~WaveGenerator_DacClk_CLKEN_MASK);
	WaveGenerator_DacClk_CLKSTBY &= (uint8)(~WaveGenerator_DacClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: WaveGenerator_DacClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((WaveGenerator_DacClk_CLKEN & WaveGenerator_DacClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = WaveGenerator_DacClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = WaveGenerator_DacClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* WaveGenerator_DacClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(WaveGenerator_DacClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(WaveGenerator_DacClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: WaveGenerator_DacClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: WaveGenerator_DacClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = WaveGenerator_DacClk_GetSourceRegister();
    uint16 oldDivider = WaveGenerator_DacClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = WaveGenerator_DacClk_CLKEN & WaveGenerator_DacClk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(WaveGenerator_DacClk_DIV_PTR, clkDivider);
                WaveGenerator_DacClk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                WaveGenerator_DacClk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(WaveGenerator_DacClk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((WaveGenerator_DacClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(WaveGenerator_DacClk_DIV_PTR, clkDivider);
				WaveGenerator_DacClk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: WaveGenerator_DacClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(WaveGenerator_DacClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: WaveGenerator_DacClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    WaveGenerator_DacClk_MOD_SRC |= modeBitMask & (uint8)WaveGenerator_DacClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: WaveGenerator_DacClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    WaveGenerator_DacClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(WaveGenerator_DacClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: WaveGenerator_DacClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return WaveGenerator_DacClk_MOD_SRC & (uint8)(WaveGenerator_DacClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: WaveGenerator_DacClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = WaveGenerator_DacClk_GetDividerRegister();
    uint8 oldSrc = WaveGenerator_DacClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        WaveGenerator_DacClk_MOD_SRC |= CYCLK_SSS;
        WaveGenerator_DacClk_MOD_SRC =
            (WaveGenerator_DacClk_MOD_SRC & (uint8)(~WaveGenerator_DacClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: WaveGenerator_DacClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return WaveGenerator_DacClk_MOD_SRC & WaveGenerator_DacClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: WaveGenerator_DacClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    WaveGenerator_DacClk_PHASE = clkPhase & WaveGenerator_DacClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: WaveGenerator_DacClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return WaveGenerator_DacClk_PHASE & WaveGenerator_DacClk_PHASE_MASK;
}
